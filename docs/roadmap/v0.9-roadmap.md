...
- [x] A2A language/runtime surface is available in `LLM-lang` (`A2A` effect + `std/a2a` docs/runtime support).
- [x] Language runtime emits `agent.*` observability events for orchestration builtins; runtime tracks these via existing agent event surfaces.
